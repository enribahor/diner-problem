...
/*	Función de afinidad curiosa que agrupa mucho la afinidad, lo que perjudica las 
	podas por estimación optimista. Además la solución es conocida, pues agrupa los
	valores pequeños alrededor del cero, y los grandes entorno al valor máximo  N  	*/
...
//Función de afinidad que devuelve el valor cargado en la matrizAfinidad
...
void mesaRedonda( int solucionFinal[], int &acumuladoFinal, int &nodosVisitados){
	...
	int solucion[N]; //	Array que lleva la solución que se está explorando
	...
	primer comensal ya colocado.
		-n: Número de comensales que se van a sentar.
...
/*	Sienta al comensal i en la posición de la mesa actual, actualizando la mesa */
...
/*	Levanta al comensal i de la posición actual	*/
...
/*	Comprueba que la solución que se va a explorar sea válida	 */
bool esValida(bool comensales[], int i){
	//Si el comensal i está a true, es que aún no está sentado y la exploración es factible
	return comensales[i];
}

/*	Indica si la solución actual es mejor que la que teníamos	*/
...
void mesaRedonda(bool comensales[], int n, int acumulado, int solucion[], int actual,
	...
	for(int i = 1; i < n; i++){
		if( esValida(comensales, i) )	/*Si el comensal ya está en la mesa sentado, se pasa al siguiente*/{		
			...
			//Si la mesa aun no está llena, llamamos de nuevo con la posición siguiente
